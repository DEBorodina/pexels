...
const MainHeaderView:React.FC<MainHeaderProbs> = ({categories,mainPicture:{src,photographer,photographer_url,alt,id},loadSrc}) => {
    return (
      ...
      <img className={styles.backgroundImage+(alt?'':" "+styles.remove)} src={src?src[loadSrc]:""} alt={alt?String(alt):""} id={String(id)}/>
        ...
        <div className={styles.trendingContainer}>
          ...
          <ul className={styles.trendingList}>
            {categories.map(({id,name},index)=>
                <li key={id} className={styles.trendingListItem} ><Link to={`/pexels/search/${name}`} className={styles.trendingListItemText} >{name}</Link><span className={styles.trendingListItemText} >{index==6?"":","}&nbsp;</span></li>
              )}
          </ul>
        </div>
